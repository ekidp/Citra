...
<body>
	...
	<table id="dg" title="Master Barang" class="easyui-datagrid" style="width:700px;height:250px"
			url="get_users.php"
	...
	<div id="dlg" class="easyui-dialog" style="width:400px;height:280px;padding:10px 20px"
		...
		<form id="fm" method="post" novalidate>
			...
			<input name="kodebarang" type="hidden">
	...
	<script type="text/javascript">
		...
		function editUser(){
			...
			if (row){
				...
				$('#fm').form('load',row);
				url = 'update_user.php?kodebarang='+row.kodebarang;
			}
		}
		function saveUser(){
			$('#fm').form('submit',{
		...
		function destroyUser(){
			...
			if (row){
				$.messager.confirm('Confirm','Yakin ate dihapus?',function(r){
					if (r){
						$.post('destroy_user.php',{kodebarang:row.kodebarang},function(result){
							if (result.success){
								$('#dg').datagrid('reload');	// reload the user data
							} else {
